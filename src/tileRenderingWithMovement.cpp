...
int main() {
    ...
    while (window.isOpen()) {
        ...
        // Processing
        cameraPos += cameraMoveSpeed * cameraMoveSpeedMultiplier;
        mousePos = sf::Mouse::getPosition(window);
        mousePos.x -= fmod(cameraPos.x, 64);
        mousePos.y -= fmod(cameraPos.y, 64);
        mousePos /= 64;
        highlighter.setPosition(mousePos.x*64.f+fmod(cameraPos.x, 64), mousePos.y*64.f+fmod(cameraPos.y, 64));
